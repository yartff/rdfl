...
#define		PTYPE(p)	((struct s_acc_getallcontent_data *)p)
struct		s_acc_getallcontent_data {
  void		*ptr;
  size_t	offset;
  size_t	c;
};
...
static
int
_cb__acc_getallcontent(void *ptr, size_t s, void *data) { // rdfl_acc_getcontent
...
void *
rdfl_acc_getcontent(t_rdfl *obj, ssize_t *sp, size_t c, e_acc_options e) {
  struct s_acc_getallcontent_data	data;
  int					extra_char, ret;
  ...
  if (!(data.ptr = malloc(data.c + extra_char))) {
    if (sp) *sp = ERR_MEMORY_ALLOC;
    return (NULL);
  }
  data.offset = 0;
  if ((ret = _iterate_chunk(obj, &_cb__acc_getallcontent, &data, e)) < 0) {
    free(data.ptr);
    if (sp) *sp = ret;
    return (NULL);
  }
  if (extra_char) ((char *)data.ptr)[data.offset] = 0;
  if (sp) *sp = data.c + extra_char;
  return (data.ptr);
}
